...
function getresponseDetails(response1) {
    fetch(`${API_URL}/discover/movie?api_key=${API_KEY}&page=${$randomid()}&with_genres=${TableauGenre[genre]}`).then((response)=>response.json()
        ...
        $movieImage.setAttribute("src", IMAGES_URL + response.results[randomget].poster_path);
        ...
        $movieImage.addEventListener("click", function() {
            ...
            $playerImage.setAttribute("src", IMAGES_URL + response.results[randomget].poster_path);
            $playerLanguage.innerText = "langue d'origine : " + response.results[randomget].original_language;
            $playerReleaseDate.innerText = "date de sortie : " + response.results[randomget].release_date;
...
//# sourceMappingURL=index.60a1ce19.js.map
